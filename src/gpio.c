/*****************************************************************************
 *   gpio.c:  GPIO C file for NXP LPC11Uxx Family Microprocessors
 *
...
void GPIOSetDir( uint32_t portNum, uint32_t bitPosi, uint32_t dir )
{
  ...
  bit value is out of range. */
	if(dir)LPC_GPIO->DIR[portNum] |= (1<<bitPosi);
	else LPC_GPIO->DIR[portNum] &= (~(0xFF&((1<<bitPosi))));
  return;
}
...
void GPIOSetValue( uint32_t portNum, uint32_t bitPosi, uint32_t bitVal )
{
	 ...
	 */
				 ...
				 if(bitVal)LPC_GPIO->SET[portNum] |= (1<<bitPosi);
				 else LPC_GPIO->CLR[portNum] |= (1<<bitPosi);
	}
  return;
}
...
int GPIOGetValue( uint32_t portNum, uint32_t bitPosi)
{
	//THIS PART IS RIPPED OFF FROM SETVALUE
	/*int i=0;
		//(LPC_GPIO0->DATA & (1 << CO2_BIT_SDA));
	  switch ( portNum )
	  {
		case PORT0:
			i=LPC_GPIO0->DATA & (1<<bitPosi);
		break;
		case PORT1:
			i=LPC_GPIO1->DATA & (1<<bitPosi);
		break;
		case PORT2:
			i=LPC_GPIO2->DATA & (1<<bitPosi);
		break;
		case PORT3:
			i=LPC_GPIO3->DATA & (1<<bitPosi);
		break;
		default:
		  break;
	}
	...
	*/
    ...
    //Mahaviksitud ja moditud
    /*int i=0;
    		//(LPC_GPIO0->DATA & (1 << CO2_BIT_SDA));
    	  switch ( portNum )
    	  {
    		case PORT0:
    			i=LPC_GPIO0->MASKED_ACCESS[(1<<bitPosi)];
    		break;
    		case PORT1:
    			i=LPC_GPIO1->MASKED_ACCESS[(1<<bitPosi)];
    		break;
    		case PORT2:
    			i=LPC_GPIO2->MASKED_ACCESS[(1<<bitPosi)];
    		break;
    		case PORT3:
    			i=LPC_GPIO3->MASKED_ACCESS[(1<<bitPosi)];
    		break;
    		default:
    		  break;
    	}
	...
	//some garbage adjusted for LPC11Uxx
	if ((LPC_GPIO->MPIN[portNum])&(1<<bitPosi))
           return 1;
